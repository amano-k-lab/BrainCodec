...
from audiocraft.data.audio_dataset import find_audio_files, load_audio_meta
...
def process_dataset(args, n_shards: int, node_index: int, task_index: tp.Optional[int] = None):
    ...
    for idx, line in tqdm.tqdm(enumerate(lines)):
        ...
        if not root_path.endswith("/"):
            root_path += "/"
        assert path.startswith(str(root_path)), f"Mismatch between path and provided root: {path} VS {root_path}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--memory_gb", type=int, help="Memory in GB.")
    parser.add_argument(
        "--format",
        type=str,
        default="wav",
