...
app = Flask(
    "mos",
    static_folder=str(magma_root / "scripts/static"),
...
def survey(signature):
    ...
    exclude_prompted = request.args.get("exclude_prompted", "false") in [
        "true",
        "on",
        "True",
    ]
    exclude_unprompted = request.args.get("exclude_unprompted", "false") in [
        "true",
        "on",
        "True",
    ]
    ...
    experiments = [
        {
            "xp": xp,
            "name": names[idx],
            "epoch": list(matched_samples.values())[0][idx].epoch,
        }
    ...
    if request.method == "POST":
        ...
        for id in model_ids:
            ...
            result = {"id": id, "is_prompted": models[0]["is_prompted"], "models": {}}
    ...
    return render_template(
        ...
        errors=[],
        ref_name=ref_name,
        already_filled=result_file.exists(),
    )
