...
from ..modules.conditioners import (ConditioningAttributes,
                                    JointEmbedCondition, WavCondition)
from ..utils.utils import warn_once
from .info_audio_dataset import (AudioInfo, InfoAudioDataset, get_keyword,
                                 get_keyword_list, get_string)
...
def augment_music_info_description(
    ...
    if random.uniform(0, 1) < merge_text_p:
        meta_pairs = [
            f"{_field.name}: {process_value(getattr(music_info, _field.name))}"
            for _field in fields(music_info)
            if is_valid_field(_field.name, getattr(music_info, _field.name))
        ]
...
class Paraphraser:
    def __init__(self, paraphrase_source: tp.Union[str, Path], paraphrase_p: float = 0.0):
...
class MusicDataset(InfoAudioDataset):
    ...
    def __init__(
        ...
        *args,
        ...
        paraphrase_source: tp.Optional[str] = None,
