...
class ViSQOL:
    ...
    def __init__(
        ...
        assert Path(
            self.visqol_model
        ).exists(), f"Could not find the specified model in ViSQOL install: {self.visqol_model}"
    ...
    def _get_target_sr(self, mode: str) -> int:
        ...
        if mode not in ViSQOL.SAMPLE_RATES_MODES:
            raise ValueError(f"Unsupported mode! Allowed are: {', '.join(ViSQOL.SAMPLE_RATES_MODES.keys())}")
