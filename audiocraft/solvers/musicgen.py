...
from ..data.music_dataset import AudioInfo, MusicDataset, MusicInfo
from ..modules.conditioners import (JointEmbedCondition, SegmentWithAttributes,
                                    WavCondition)
from ..utils.cache import CachedBatchLoader, CachedBatchWriter
            ...
            for info in infos:
                if isinstance(info, MusicInfo):
                    ...
                    if dataset.paraphraser is not None and info.description is not None:
                        ...
                        info.description = dataset.paraphraser.sample_paraphrase(info.meta.path, info.description)
        # prepare attributes
        ...
        for batch in lp:
            ...
            if self.cfg.generate.lm.unprompted_samples:
                if self.cfg.generate.lm.gen_gt_samples:
                    ...
                    self.logger.warn("Use ground truth instead of audio generation as generate.lm.gen_gt_samples=true")
    ...
    def evaluate_audio_generation(self) -> dict:
        ...
        if self.cfg.evaluate.metrics.chroma_cosine:
            ...
            has_predefined_eval_chromas = (
                ...
                and self.model.condition_provider.conditioners["self_wav"].has_eval_wavs()
            )
        ...
        if should_run_eval:
            ...
            for idx, batch in enumerate(lp):
                ...
                gen_outputs = self.run_generate_step(batch, gen_duration=target_duration, **self.generation_params)
