...
class DiffusionSolver(base.StandardSolver):
    ...
    def __init__(self, cfg: omegaconf.DictConfig):
        ...
        self.codec_model = CompressionSolver.model_from_checkpoint(cfg.compression_model_checkpoint, device=self.device)
        ...
        assert self.codec_model.sample_rate == self.sample_rate, (
            f"Sample rate of solver {self.sample_rate} and codec {self.codec_model.sample_rate} " "don't match."
        )
        ...
        self.schedule = NoiseSchedule(**cfg.schedule, device=self.device, sample_processor=self.sample_processor)
