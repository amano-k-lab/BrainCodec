...
def get_solver(cfg: omegaconf.DictConfig) -> StandardSolver:
    ...
    from .downstream_CSM_fMRI import DownstreamCSMfMRI
    ...
    klass = {
        ...
        "compression_fMRI": CompressionfMRISolver,
        "downstream_fMRI": DownstreamfMRISolver,
        "downstream_CSM_raw_fMRI": DownstreamCSMrawfMRI,
        "downstream_CSM_fMRI": DownstreamCSMfMRI,
        "musicgen": MusicGenSolver,
        "audiogen": AudioGenSolver,
        "CSM_raw_fMRI": CSMrawfMRI,
        "CSM_fMRI": CSMfMRI,
...
def get_adversary(name: str, cfg: omegaconf.DictConfig) -> nn.Module:
    ...
    klass = {
        ...
        "msdfMRI": adversarial.fMRIMultiScaleDiscriminator,
...
def get_fMRI_datasets(
    cfg: omegaconf.DictConfig, dataset_type: DatasetType = DatasetType.FMRI
) -> tp.Dict[str, torch.utils.data.DataLoader]:
    """Build fMRIDataset from configuration.
    ...
    for split, path in cfg.datasource.items():
        ...
        if execute_only_stage is not None and split != execute_only_stage:
            continue
        logger.info(f"Loading fMRI data split {split}: {str(path)}")
        ...
        return_info = kwargs.pop("return_info")
        assert return_info is True, "return_info must be True for fMRI dataset"
        batch_size = kwargs.pop("batch_size", None)
        num_workers = kwargs.pop("num_workers")

        if dataset_type == DatasetType.FMRI:
            dataset = data.fMRI_dataset.fMRIDataset.from_meta(path, **kwargs)
        else:
            raise ValueError(f"Dataset type is unsupported: {dataset_type}")
