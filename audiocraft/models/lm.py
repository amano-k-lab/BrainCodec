...
class LMModel(StreamingModule):
    ...
    def compute_predictions(
        ...
        (
            ...
            sequence_indexes,
            sequence_mask,
        ) = pattern.build_pattern_sequence(codes, self.special_token_id, keep_only_valid_steps=True)
...
class LMModelfMRI(LMModel):
        ...
        if for_lm is False:
            ...
            for idx, (_tr, bold_d) in enumerate(zip(raw_tr, bold_durations)):
                ...
                if bold_d > max_durtion:
                    # max_duration はあくまでも segment size
                    # たまに segment size を超えるやつはいる．
            ...
            # Difference: add pad in the start & end
            # max_duration には pattern で変形したあとのサイズがくる．
            # 訓練時は +1, 生成時はさらに delay 分だけ増えている
            for idx, (_tr, bold_d) in enumerate(zip(raw_tr, bold_durations)):
                assert bold_d % 3 == 0
                _tr_tensor = (torch.arange(0, bold_d // 3) * _tr).unsqueeze(-1).repeat(1, 3).view(-1)
                _tr_tensor = (_tr_tensor / self.tr_precision).floor()
                # add token at the start ↓
                _tr_tensor = torch.cat((self.tr_emb_pad_id * torch.ones(1), _tr_tensor))
                if len(_tr_tensor) > max_durtion:
                    _tr_tensor = _tr_tensor[:max_durtion]
                else:
                    pad_length = max_durtion - len(_tr_tensor)
                    _tr_tensor = torch.cat((_tr_tensor, self.tr_emb_pad_id * torch.ones(pad_length)))
                tr[idx] = _tr_tensor
    ...
    def _sample_next_token(
        ...
        if self.two_step_cfg and cfg_conditions != {}:
            assert isinstance(cfg_conditions, tuple), type(cfg_conditions)
            condition_tensors, null_condition_tensors = cfg_conditions
            cond_logits = model(sequence, tr_seq, conditions=[], condition_tensors=condition_tensors)
            state = self.get_streaming_state()
            self.set_streaming_state(unconditional_state)
            uncond_logits = model(sequence, conditions=[], condition_tensors=null_condition_tensors)
            unconditional_state.update(self.get_streaming_state())
            ...
            if condition_tensors:
                # Preparing for CFG, predicting both conditional and unconditional logits.
                sequence = torch.cat([sequence, sequence], dim=0)
            all_logits = model(sequence, tr_seq, conditions=[], condition_tensors=condition_tensors)
            if condition_tensors:
                cond_logits, uncond_logits = all_logits.split(B, dim=0)  # [B, K, T, card]
                logits = uncond_logits + (cond_logits - uncond_logits) * cfg_coef
            else:
    ...
    def generate(
        ...
        # it is the first sequence step that contains the `start_offset` timestep
        start_offset_sequence = pattern.get_first_step_with_timesteps(start_offset)
        tr_tensor = self.tr_preprocess(tr, gen_sequence.shape[-1], [max_gen_len] * num_samples, for_lm=True).to(device)
        assert start_offset_sequence is not None
